...
public class MainFragment extends Fragment {
    ...
    // 프래그먼트 화면 붙이기 : 반드시 onCreateView를 오버라이드 해야 함
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.fragment_main, container, false);

        //fragment_main => rootView에 저장되었기 때문에
        rootView.findViewById(R.id.btnMain).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // 자신이 속한 Activity를 알게한다 : 여기서는 메인 액티비티
                activity = (MainActivity) getActivity();   //내가 어디 있구나 알 수 있음. 캐스팅
